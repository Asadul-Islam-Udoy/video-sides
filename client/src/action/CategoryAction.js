...
export const getAllCategoryVideoAction = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("https://video-sides.onrender.com/api/categories/get/all");
...
export const getSinglesCategoriesVideoAction = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://video-sides.onrender.com/api/categories/get/single/categories/videos/${id}`);
